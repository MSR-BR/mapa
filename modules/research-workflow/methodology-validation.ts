...
export const methodologyPlanInputSchema = z.object({
  ...
  rows: z.array(methodologyRowInputSchema).min(3).max(6),
...
export function validateMethodologyPlan(
  ...
  if (objectiveSet.size !== objectiveIds.length) {
    errors.push("Cada objetivo específico deve aparecer em uma única linha da matriz.");
  }
  ...
  for (const objectiveId of objectiveSet) {
    if (!options.allowedObjectiveIds.has(objectiveId)) errors.push("A matriz aponta para objetivo específico inexistente.");
  }
