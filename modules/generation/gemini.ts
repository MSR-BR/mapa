...
export async function suggestResearchPrompts(prompt: string) {
  const { output } = await generateText({
    ...
    prompt: [
      ...
      "Retorne três alternativas numeradas implicitamente pela ordem: Tema 1, Tema 2 e Tema 3.",
      "Não use os rótulos 'Tema de pesquisa', 'Investigar' ou 'Analisar' no início das sugestões.",
      "A primeira alternativa deve preservar o tema com um recorte relevante e coerente com a intenção já escrita.",
      "A segunda deve reescrever o pedido com maior precisão acadêmica e a terceira sugestão de recorte deve delimitar a investigação sem inventar contexto.",
