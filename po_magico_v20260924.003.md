...
PÓ MÁGICO VERSION: `v20260924.003`

PARENT: `po_magico_v20260924.002.md`

EVOLUTION SCOPE: Digital Discovery & Growth blueprint v1.3, canonical-consolidation proof, crawlable noindex governance, provider readback, and production performance sampling
...
Transversal blueprint:

- DIGITAL_DISCOVERY_AND_GROWTH

This transversal blueprint may support APP, SOFTWARE, AI_SYSTEM, EDUCATIONAL_MATERIAL, REPORT, or GENERIC work. It does not replace the primary deliverable.
...
### APP BLUEPRINT DELTA — v2.3 (validated from production RLS activation)

The following controls extend the default APP workflow:

1. **Reproduce the exact client write semantics.** A bare SQL `INSERT` or
   `UPDATE` does not prove that the application's request will pass. Test the
   same authenticated operation, selected representation, `RETURNING` clause,
   nested read, and response shape emitted by the real client library.
2. **Run a synthetic authenticated production matrix before enabling a flag.**
   Use isolated test accounts for each role, exercise positive and negative
   ownership paths, restore their original state, remove temporary records, and
   keep the feature fail-closed until every critical assertion passes.
3. **Treat a failed pre-activation check as a successful safety gate.** Correct
   published database behavior only through an explicit roll-forward migration,
   rerun local and remote matrices, and activate the feature only after the
   corrected path passes.
4. **Classify platform-advisor output instead of counting warnings.** Errors
   block activation. Intentional warnings require a documented threat model,
   internal authorization controls, least-privilege grants, operational
   tradeoff, and an owner for any remaining platform setting.
5. **Preserve independent rollback layers.** A feature flag can hide the user
   path without reverting database hardening. Policy defects require audited
   forward migrations; destructive resets and edits to applied migrations are
   not rollback strategies.

**Version record**

- Previous APP blueprint: v2.2.
- New APP blueprint: v2.3.
- Triggering project: Mapa da Pesquisa, Change 087.
- Adopted lesson: production RLS must be exercised through the exact
  authenticated client contract before activation.
- Reason: a direct database insert passed while the app-equivalent
  `INSERT ... RETURNING` exposed a same-command visibility defect.
- Confidence: HIGH.


### APP BLUEPRINT DELTA — v2.4 (validated from switchable-role E2E rollout)

The following controls extend the default APP workflow:

1. **Separate identity, active mode, authorship, and collaboration.** Account
   identity remains stable, active mode is versioned and persistent, resource
   authorship is immutable, and collaboration is an explicit relationship.
   Switching modes must never reclassify existing resources.
2. **Use reversible synthetic fixtures for authorization journeys.** Exercise
   both modes with distinct authenticated accounts and two sessions/tabs,
   preserve initial state, force stale-version conflicts, test direct API/RLS
   denial, and remove temporary resources in guaranteed cleanup.
3. **Layer browser evidence without hiding automation gaps.** Verify public
   load, authentication, rendered DOM, interaction contracts, Server
   Actions/APIs, and database policies independently. A harness timeout is
   neither product failure nor acceptance success until the failing layer is
   isolated and documented.
4. **Prove every delivered export format end to end.** A generator alone is not
   a shipped feature. Validate visible controls, authorized routes, allowed
   workflow states, private response headers, filenames, and binary signatures
   for every relevant user mode.
5. **Keep DNS cutovers outside app/database rollout.** When third-party DNS is
   already functional, preserve it by default. Inspect domain assignment,
   NS/A/CNAME/MX/TXT, HTTPS/HSTS, and deliberate host omissions read-only;
   require explicit authority, zone inventory, rollback, and a separate window
   before any delegation.
6. **Reconcile release metadata after deployment.** Confirm that environment
   overrides do not leave the health/version identifier behind the deployed
   commit. A READY deployment with stale release metadata is not final closure.

**Version record**

- Previous APP blueprint: v2.3.
- New APP blueprint: v2.4.
- Triggering project: Mapa da Pesquisa, Change 088.
- Adopted lesson: switchable-role closure requires reversible cross-layer proof,
  format-level validation, and infrastructure gates that remain independent.
- Reason: the rollout passed database and API matrices, exposed a dormant DOCX
  route gap, isolated headless-driver false negatives, and caught a stale
  Production version override before closure.
- Confidence: HIGH.

### APP BLUEPRINT DELTA — v2.5 (validated from cross-layer human-approval rollout)

The following controls extend the default APP workflow:

1. **Separate editable draft from protected progression.** Define explicitly
   which edits, saves, backward navigation, and recovery actions remain
   available while human approval is missing. Block only the protected forward
   transition unless the product requirement says otherwise.
2. **Enforce one gate across interface, server, and database.** A disabled
   button is guidance, not authorization. Use a stable machine-readable server
   error and a database policy or trigger for direct-client bypass resistance.
3. **Validate transition intent, not only state rank.** A pending approval must
   bind the current step, target state, target step, reviewer identity, and
   status. Approval advances atomically; the resource owner cannot impersonate
   the reviewer or skip the pending state.
4. **Prove both denial and authorized success.** With reversible synthetic
   accounts, test missing reviewer, direct owner bypass, linked review,
   reviewer-only approval, autonomous expert-owned resources, and guaranteed
   cleanup. Confirm that a denied attempt leaves persisted state unchanged.
5. **Treat authenticated product copy as policy evidence.** Run a rendered
   smoke for each role after deployment. Language such as optional, required,
   owner, reviewer, or supervisor must match the enforced policy; contradictory
   copy is a release defect even when the security control is correct.
6. **Make privileged database access a rollout dependency.** Verify the exact
   organization and project before execution. If CLI privileges fail, stop that
   path; use an authorized console only with exact artifact comparison,
   transactional execution, and post-application readback. Never expose or
   recover secrets as a workaround.

**Version record**

- Previous APP blueprint: v2.4.
- New APP blueprint: v2.5.
- Triggering project: Mapa da Pesquisa, Change 089.
- Adopted lesson: human approval flows need consistent progression semantics and
  independently verified controls in UX, API, and database layers.
- Reason: production verification proved the database denial and authorized
  success, while the authenticated smoke found policy-copy drift that static
  security checks alone did not reveal.
- Confidence: HIGH.

### APP BLUEPRINT DELTA — v2.6 (validated from shared UI microcopy release)

The following controls extend the default APP workflow:

1. **Give repeated interface copy one canonical owner.** Before changing text,
   identify the shared component or source used by every relevant surface. Edit
   that owner once, inventory its consumers, and avoid page-specific duplicates
   that can drift.
2. **Treat copy-only work as a behavior-preservation contract.** Record the exact
   requested string and explicit non-goals. Assert both the new text and the
   absence of the retired text while retaining interaction, validation,
   persistence, accessibility, and navigation tests.
3. **Verify hidden or collapsible copy in the rendered production state.** Open
   the control that reveals the content and inspect the actual DOM on relevant
   responsive widths. Source inspection or initial HTML alone is insufficient
   when the component is client-rendered or initially collapsed.
4. **Use layered artifact evidence when browser tooling is constrained.** A
   production bundle check can prove which literal was shipped, but it should
   supplement a rendered interaction smoke whenever a safe headless or manual
   path is available. Record tooling limitations instead of claiming visual
   evidence that did not run.

**Version record**

- Previous APP blueprint: v2.5.
- New APP blueprint: v2.6.
- Triggering project: Mapa da Pesquisa, Change 090.
- Adopted lesson: exact microcopy changes in shared interactive components need
  canonical ownership, negative assertions, and rendered responsive proof.
- Reason: the field was initially hidden inside a collapsed client component;
  code and initial HTML checks alone could not prove the user-visible state.
- Confidence: HIGH.

### APP BLUEPRINT DELTA — v2.7 (validated from DNS and email-authentication hardening)

The following controls extend the default APP workflow:

1. **Treat external DNS as a minimal, separately authorized release.** Confirm
   the authenticated account, registrant authority, exact FQDN, current zone,
   intended single-record delta, TTL, rollback, and affected services before
   writing. Never modify another visible domain or combine DNS changes with an
   unrelated app/database deployment.
2. **Use read-before-write and full readback.** Inventory the zone before the
   mutation, submit only the authorized delta, re-read the complete zone, and
   prove that unrelated A/AAAA/CNAME/MX/TXT/CAA/DS records remain intact.
   Provider UI confirmation alone is not closure evidence.
3. **Prove propagation at both authority and recursion layers.** Query every
   authoritative nameserver and at least one independent recursive resolver.
   Keep the application/site health check separate from DNS propagation and
   record exact timestamps and rollback scope.
4. **Harden email authentication incrementally.** Before adding DMARC, verify
   MX, DKIM, SPF, Return-Path and receiving routes. Do not add an apex SPF when
   the sender correctly authenticates a technical subdomain. Begin DMARC at
   `p=none`, collect aggregate reports, and require a new Change before
   `quarantine` or `reject`.
5. **Keep production secrets in production during verification.** Prefer an
   already-deployed server-side path for functional smoke instead of exporting
   hidden provider keys to a local runner. A missing local secret is an
   expected boundary, not a reason to weaken secret handling.
6. **Realign verification after authentication-provider cutovers.** When Email
   or password login is retired, update remote synthetic runners to a supported
   authentication path or document an equivalent layered proof. Never
   reactivate a retired provider solely to make a legacy E2E script pass.
7. **Distinguish UI failure from authorization failure.** If the provider UI
   stalls, diagnose session, anti-CSRF and API responses in read-only mode.
   Any fallback write must use the provider's own authenticated endpoint,
   exact payload, least mutation, and immediate readback; generic browser or
   credential workarounds are not acceptable.

**Version record**

- Previous APP blueprint: v2.6.
- New APP blueprint: v2.7.
- Triggering project: Mapa da Pesquisa, Changes 093–095.
- Adopted lesson: DNS and email-authentication changes require account/FQDN
  confirmation, zone-preserving writes, authoritative and recursive proof,
  phased enforcement, and production-secret-safe smoke tests.
- Reason: the registrar UI stalled on an XSRF request while the authorized
  session and official endpoint remained healthy; layered verification safely
  published one DMARC record without changing routing or application code.
- Confidence: HIGH.
...
## 5.14 TRANSVERSAL DIGITAL DISCOVERY & GROWTH BLUEPRINT

Use alongside a primary blueprint when a project needs discoverability, SEO, index governance, analytics, organic distribution, conversion measurement, or paid-acquisition readiness.

This blueprint is transversal. It must not replace the APP, SOFTWARE, ARTICLE, EDUCATIONAL MATERIAL, REPORT, or other primary blueprint that owns the actual deliverable.

Internal dimensions:

- audience and search intent;
- canonical destinations and indexability;
- content and distribution surfaces;
- technical SEO and performance;
- Search Console ownership and baseline;
- analytics contract and attribution integrity;
- activation, value, and retention outcomes;
- consent and privacy governance;
- campaign taxonomy and economics;
- experiments, monitoring, and learning.

### DIGITAL DISCOVERY & GROWTH METHODOLOGY — v1.3

#### Gate A — proposition, audience, and intent

Define the audience, problem, value proposition, primary intent, useful destination, and core action. Separate brand discovery, educational discovery, product activation, and paid campaign intent. One page must not be forced to serve incompatible intents without an explicit information architecture.

#### Gate B — canonical discovery foundation

Before promotion, verify canonical host, redirects, indexable routes, noindex controls, robots, sitemap, metadata, social cards, structured data, internal links, mobile rendering, and performance. Use one canonical URL per intent. A sitemap, canonical tag, and redirect must not contradict one another.

Close a canonical migration only after an end-to-end proof shows:

- the retired URL returns a permanent redirect to the intended destination;
- the destination emits the expected self-canonical;
- the sitemap contains only the canonical destination;
- robots allows the canonical destination;
- utility or private HTML routes emit noindex through metadata or headers while
  remaining crawlable enough for the directive to be read;
- the search provider accepts the updated sitemap and its discovered-URL count
  reconciles with the published XML.

Do not use robots disallow as a substitute for noindex. For performance gates,
measure the deployed canonical URL at least twice under the target mobile
profile, report variance, and distinguish lab scores from field Core Web
Vitals. Diagnose the actual LCP element before optimizing; late font swaps,
oversized responsive images, dynamic rendering and cold-edge latency require
different remedies.

#### Gate C — property ownership and baseline

Establish Search Console and analytics property ownership. Submit the sitemap, inspect representative URLs, and record a dated baseline for indexed pages, queries, impressions, clicks, CTR, position, sessions, engagement, activation, and completion. If property-level evidence is unavailable, state that limitation instead of inferring configuration from code.

Treat property rollout as a sequence of independent, observable gates:

- confirmed account and property ownership;
- DNS verification written with read-before-write and authoritative/recursive
  readback;
- sitemap submission and successful provider processing;
- indexed-state inspection and separate live-availability testing;
- aggregate-report readiness, with unavailable values kept distinct from zero;
- analytics association to the exact property and web stream, read back in both
  products.

If a newly submitted sitemap temporarily reports a fetch failure, first verify
public HTTP status, content type, robots access, Googlebot access, XML validity,
and provider readback before removing or repeatedly resubmitting it. Record the
initial and final states. Inspect both user-declared and provider-selected
canonicals, because live availability does not prove index inclusion and an
unknown URL is not equivalent to a crawl failure.

#### Gate D — measurement integrity

Separate acquisition dimensions from product context. Never reuse source, medium, campaign, term, or content as generic app parameters. Prefix controlled product parameters consistently. Keep PII, identifiers, prompts, free text, documents, and high-cardinality values out of analytics.

Choose one to three primary KPIs, then define diagnostic metrics and guardrails. Mark durable outcomes, not incidental clicks, as key events. Reproduce a known production journey in DebugView and in the final funnel before trusting the data.

When repairing an active analytics implementation, preserve stable event names whenever their semantics remain valid, move product context to a clearly prefixed namespace, and register successor custom dimensions without deleting legacy definitions. Record the migration date as a series break. Validate one controlled production journey, confirm event cardinality and acquisition fields, then observe a clean baseline before making pre/post claims.

#### Gate E — consent and governance

Document the consent implementation, regional behavior, revocation path, retention, internal traffic, reporting identity, owners, and change history. Analytics consent and advertising consent are separate decisions. Future advertising requires explicit treatment of analytics_storage, ad_storage, ad_user_data, and ad_personalization.

#### Gate F — organic distribution

Map audience questions and search intents to genuinely useful pages, demonstrations, reports, videos, or educational assets. Prefer people-first content and stable landing destinations. Do not manufacture low-value pages for keyword volume. Every asset must have an owner, update cadence, CTA, measurement plan, and retirement rule.

#### Gate G — paid-media readiness

Do not link, import conversions, or spend until canonical destinations, source/medium integrity, at least one verified key event, consent, internal-traffic control, mobile performance, UTM standards, budget, geography, negative keywords, and stop-loss are approved. Begin with one primary conversion and the smallest reversible test.

#### Gate H — learning and controlled scaling

Annotate every taxonomy or landing-page change. Observe a clean baseline window before comparing cohorts. Review acquisition quality, activation, completion, cost, conversion lag, support load, privacy, and failure rates together. Scale only when the core outcome improves without violating guardrails.

Typical Change logic:

    001-audit-discovery-index-and-measurement
    002-repair-attribution-and-outcome-events
    003-establish-search-ownership-and-baseline
    004-consolidate-canonical-pages-and-performance
    005-build-organic-distribution-assets
    006-verify-consent-and-paid-media-readiness
    007-run-a-small-approved-acquisition-test
    008-monitor-learn-and-evolve-the-blueprint

Version record:

- Previous blueprint: v1.2.
- New blueprint: v1.3.
- Triggering project: Mapa da Pesquisa, Change 099.
- Adopted lesson: canonical consolidation requires aligned redirect, canonical,
  sitemap, robots, crawlable noindex, provider readback and repeated production
  performance evidence; one local test or one Lighthouse run is insufficient.
- Reason: code-level correctness did not prove provider convergence, robots
  blocking would have hidden noindex, and repeated mobile samples exposed
  font/image and network variance that a single score obscured.
- Confidence: HIGH.
...
DIGITAL_DISCOVERY_AND_GROWTH BLUEPRINT VERSION
```
